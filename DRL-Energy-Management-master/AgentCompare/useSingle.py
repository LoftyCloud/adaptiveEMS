...
# ######################加载工况数据######################### 读取MAT文件
DC1 = scipy.io.loadmat('../Data_Standard Driving Cycles/Standard_HWFET.mat')
DC2 = scipy.io.loadmat('../Data_Standard Driving Cycles/Standard_UDDS.mat')
DC3 = scipy.io.loadmat('../Data_Standard Driving Cycles/Standard_NEDC.mat')
# 获取速度向量，并进行单位转换
...
for j in range(len(car_spd_one) - 1):
    a1 = actor1(np_to_tensor(s)).detach().numpy()
    a2 = actor2(np_to_tensor(s)).detach().numpy()
    if AgentName == "AgentS":
        Eng_pwr_opt = (a1[0]) * 56000
    elif AgentName == "AgentU":
        # print(1)
        Eng_pwr_opt = (a2[0]) * 56000
    else:
        pass
    # Eng_pwr_opt = (a2[0]) * 56000
    # print(a)
