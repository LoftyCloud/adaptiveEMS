...
class Prius_model():
    def __init__(self):
        ...
        # 将 Eng_pwr_opt_list 映射到对应的 W_list 值（EngSpd
        ...
        data_path = 'E:\pythonProject\DRL-Energy-Management-master\Eng_bsfc_map.mat'
        ...
        data_path1 = 'E:\pythonProject\DRL-Energy-Management-master\Mot_eta_quarter.mat'
        ...
        self.Batt_Q = Batt_Q_cell * 3600  # As
        ...
        #Battery current limitations 电流限制
    ...
    def run(self, car_spd, car_a, Eng_pwr_opt, SOC):
        ...
        # Wheel speed (rad/s) 计算车轮转速
        Wheel_spd = car_spd / self.Wheel_R

        # Wheel torque (Nm) 计算车轮转矩
        ...
        # 发动机输出功率过低时不工作，且T<0时为再生制动，此时发动机也不工作
        if (Eng_pwr_opt < 500) or (T < 0):
            Eng_trq = 0
            Eng_spd = 0

        # 获取燃油消耗速率
        Eng_fuel_mdot = self.Eng_fuel_func(Eng_trq, Eng_spd)  # (g/s)
        # maximum engine torque boundary (Nm) 计算发动机当前转速下的的最大转矩边界
        Eng_trq_max = np.polyval(self.Eng_trq_maxP, Eng_spd)
        # engine power consumption # 发动机实际输出功率，燃油消耗*燃油能量
        Eng_pwr = Eng_fuel_mdot * 42600  # g/s*j/g = j/s(=W)
    ...
    #        print('battery power is out of bound')
        ...
        SOC_new = (np.conjugate(SOC_new) + SOC_new) / 2 # np.conjugate逐元素返回复共轭
        
        if SOC_new > 0.9:
            SOC_new = 0.9
        
        P_out = Eng_pwr_opt + Batt_pwr  # 实际输出功率

        # I为False
        I = (inf_batt + inf_eng + inf_mot + inf_gen != 0)
        # Calculate cost matrix (fuel mass flow)
        cost = (Eng_pwr / 42600)  # 当前时间步消耗的燃油量g
        # 仿真过程中的变量记录
        out = {}
        out['Batt_V'] = Batt_vol
        out['Batt_I'] = Batt_I
        out['P_batt'] = Batt_vol*Batt_I
