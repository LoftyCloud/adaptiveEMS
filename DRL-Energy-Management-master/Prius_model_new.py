...
class Prius_model():
    def __init__(self):
        ...
        # paramsmeters of transmission system 传动系统参数
        # number of teeth 行星齿轮组齿轮数
        self.Pgs_R = 78
        self.Pgs_S = 30
        ...
        Eng_bsfc_map = data['Eng_bsfc_map']  # 最优燃油消耗曲线
        self.Eng_trq_maxP = [-4.1757e-009, 6.2173e-006, -3.4870e-003, 9.1743e-001, 2.0158e+001]
        Eng_fuel_map = Eng_bsfc_map * (Eng_spd_list.T * Eng_trq_list) / 3600 / 1000  # 燃油消耗速率函数（kg/h）

        # fuel consumption (g)
        # 插值函数，根据引擎的转速和扭矩值，输出对应的燃料消耗速率
        ...
        Gen_trq_max_half = Gen_trq_max_half[np.newaxis, :]  # np.newaxis 增加新的维度
        ...
        Batt_Q_cell = 6.5     
        # coulombs, battery package capacity 一库仑等于一安培秒（A·s）
        ...
        # Battery current limitations 充放电电流限制
        self.Batt_I_max_dis = 196
        self.Batt_I_max_chg = 120 
        
        
    def run(self, car_spd, car_a, Eng_pwr_opt, SOC):  # 运行一步，并返回各种值变化、奖励
        # Wheel speed (rad/s)
        ...
        Eng_pwr = Eng_fuel_mdot * 42600 # 转换为焦耳
        ...
        # Calculate electric power consumption 发电机电能消耗
    ...
    #        print('battery power is out of bound')
        ...
        P_out = Eng_pwr_opt + Batt_pwr  # 输出功率 = 发动机功率 + 电机功率

        I = (inf_batt + inf_eng + inf_mot + inf_gen != 0)
        # Calculate cost matrix (fuel mass flow) 损失值
        # cost = (Eng_pwr / 42600) # 评估引擎效率或燃料效率
        cost = Eng_fuel_mdot  # 当前燃油消耗值（g）
